...
def register():
    ...
    with get_db() as db:
        ...
        db.execute(
            ...
            (username, email, password_hash, otp_code, otp_expires)
            VALUES (?, ?, ?, ?, ?)
            """,
            (uname, email, pwd_hash, otp, expires),
        )

    try:
        send_otp_email(email, uname, otp)
    except Exception as e:
        print("MAIL ERROR (register):", e)
...
def posts():
    ...
    if request.method == "POST":
        try:
            ...
            proba = gbc.predict_proba(x)[0]  # [phishing_prob, non_phishing_prob]
            ...
            with get_db() as db:
                db.execute(
                    ...
                    VALUES (?, ?, ?, ?, datetime('now'))
                    """,
                    (session.get("username"), url, label, non_phish_prob),
                )

            return render_template(
                "result.html", xx=round(non_phish_prob, 2), url=url
            )
        except Exception as e:
...
def forgot_post():
    ...
    flash(
        "We sent a reset link to your email (valid 30 minutes).",
        "success",
    )
    return redirect(url_for("login"))
